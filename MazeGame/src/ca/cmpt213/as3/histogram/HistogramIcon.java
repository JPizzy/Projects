...
public class HistogramIcon implements Icon{
	...
	private final int BAR_SPACING = 10;
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		if(numOfBars > 0) {
			...
			while(itr.hasNext()){
				...
				// Draw Text
				g2d.setColor(Color.BLACK);
				g2d.drawString(barCount, barCountOffset, startY - TEXT_HEIGHT);
				if(barIndex % 2 == 0) {
					g2d.drawString(barMinInterval, barMinIntervalOffset, barHeight + startY + (2 * TEXT_HEIGHT));
				} else {
					g2d.drawString(barMinInterval, barMinIntervalOffset, barHeight + startY + TEXT_HEIGHT);
				}
